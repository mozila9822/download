...
const categories = ['City Break', 'Hotel', 'Tour', 'Flight', 'Coach Ride']

export default function ServicesAdminPage() {
  const { toast } = useToast()
  const [category, setCategory] = useState<string>('City Break')
  ...
  }, [category, offerOnly, statusFilter, searchTerm])
  ...
  const updateService = async () => {
    if (!editItem) return
    try {
      const payload: any = { ...form, category }
    ...
    <Card>
      <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <CardDescription>Manage destinations, accommodation, tours, and more.</CardDescription>
        </div>
        <div className="flex gap-2">
          <Button onClick={openAdd} className="rounded-lg"><Plus className="h-4 w-4 mr-2" /> Add</Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && !error && filtered.length === 0 && <TableRow><TableCell colSpan={9}>No services found.</TableCell></TableRow>}
              {filtered.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <Button variant="secondary" size="sm" asChild className="rounded-md"><a href={`/admin/services/${s.id}/availability`} target="_blank" rel="noreferrer">Availability</a></Button>
                      <Button variant="secondary" size="sm" asChild className="rounded-md"><a href={`/admin/pricing?serviceId=${encodeURIComponent(s.id)}`} target="_blank" rel="noreferrer">Pricing</a></Button>
