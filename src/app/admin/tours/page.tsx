...
export default function ToursAdminPage() {
  ...
  const updateService = async () => {
    if (!editItem) return
    try {
      const payload: any = { ...form, category: 'Tour' }
    ...
    <Card>
      <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <CardDescription>Manage tour packages, availability and promotions.</CardDescription>
        </div>
        <div className="flex gap-2">
          <Button onClick={openAdd} className="rounded-lg"><Plus className="h-4 w-4 mr-2" /> Add Tour</Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && !error && filtered.length === 0 && <TableRow><TableCell colSpan={9}>No tours found.</TableCell></TableRow>}
              {filtered.map((s) => (
                <TableRow key={s.id}>
