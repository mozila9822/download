...
export async function POST(req: NextRequest) {
  try {
    ...
    const created = await client.service.create({
      data: {
...
export async function GET(req: NextRequest) {
  try {
    const client = await prismaOrNull()
    // When DB is unreachable, fall back to mock services for read-only UI
    if (!client) {
      // Keep admin auth checks even when serving mock data
      const token = req.cookies.get(SESSION_COOKIE_NAME)?.value
      const session = token ? await verifySessionToken(token) : null
      const isAdmin = session ? canAccessAdmin(String(session.role) as any) : false
      if (!isAdmin) {
      ...
      let rows = [...mockServices]
      // Filter by category (UI values like 'City Break')
      if (categoryParam && categoryParam !== 'All') {
        rows = rows.filter((s) => s.category === categoryParam)
      }
      ...
      // Filter by status (mock defaults to 'Active')
      if (status && ['Active', 'Inactive', 'Archived'].includes(status)) {
        rows = rows.filter((s) => (s.status ?? 'Active') === status)
      }
      // Filter by availability
      if (availableParam) {
        const want = availableParam.toLowerCase() === 'true'
        rows = rows.filter((s) => (s.available ?? true) === want)
      }
      // Filter offer only
      if (offerOnly) {
        rows = rows.filter((s) => s.offerPrice != null)
      }

      const dto = rows.map((s) => ({
        id: s.id,
        category: s.category, // already UI label (e.g., 'City Break')
        title: s.title,
        ...
        isOffer: (s.isOffer ?? false) || (s.offerPrice != null && Number(s.offerPrice) > 0),
      }))
