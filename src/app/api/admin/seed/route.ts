  ...
  try {
    ...
    if (existingServiceCount === 0) {
      ...
      for (const s of samples) {
        const created = await client.service.create({ data: s })
    ...
    const existingBookingCount = await client.booking.count()
    if (existingBookingCount === 0) {
      const serviceId = seededServiceIds[0] || (await client.service.findFirst({ select: { id: true } }))?.id
      if (!serviceId) {
        return NextResponse.json({ ok: false, error: 'No service available to create a booking.' }, { status: 500 })
      }

      const numberOfTravelers = 2
      const svc = await client.service.findUnique({ where: { id: serviceId } })
