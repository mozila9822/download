...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Align behavior with GET /api/users: if DB is unreachable, act as no-op success.
    const db = await prismaOrNull()
    if (!db) {
      return new NextResponse(null, { status: 204 })
    }

    // If the id is non-numeric (e.g., temp IDs from fallback creation), treat as no-op.
    const idNum = Number(idParam)
    if (!Number.isFinite(idNum)) {
      return new NextResponse(null, { status: 204 })
    }
    ...
    try {
      await db.user.delete({ where: { id: idNum } })
      return new NextResponse(null, { status: 204 })
    } catch (err: any) {
      // Record not found -> consider deletion successful for idempotency
      if (err && (err.code === 'P2025' || String(err.code) === 'P2025')) {
        return new NextResponse(null, { status: 204 })
      }
      // Foreign key constraint -> gracefully demote user out of staff directory
      const message = String(err?.message || '')
      if (err?.code === 'P2003' || message.includes('Foreign key constraint')) {
        try {
          await db.user.update({ where: { id: idNum }, data: { role: 'User' as any } })
          return new NextResponse(null, { status: 204 })
        } catch (updateErr) {
          console.error('[DELETE /api/users/:id] demote on FK failure error', updateErr)
        }
      }
      console.error('[DELETE /api/users/:id] prisma delete error', err)
      return new Response('Internal Server Error', { status: 500 })
    }
  } catch (e) {
